...
public class anadirnota extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (actionBar != null) {
            actionBar.setTitle(Html.fromHtml("<font color='#FFFFFF'>Añadir Nota</font>"));
        }
    ...
    public void volver(MenuItem item){
        ...
        builder.setNegativeButton("No", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Intent i = new Intent(anadirnota.this, MainActivity.class);
                startActivity(i);
            }
        });
    ...
    public void añadirmetodo(){
        if(db != null)
        {
            if(comprobarDatos())
            {
                try
                {
                    db.execSQL("insert into Notas( titulo, nota) values ( '" + titulo.getText().toString() + "', '" + texto.getText().toString() + "');");
                    titulo.setText("");
                    texto.setText("");
